// Preston Duffield
// This solution ensures that all threads always acquire the locks in the same order.
#include <pthread.h>
#include <err.h>
#include <time.h>
#include <stdio.h>
...
void *thread2(void *arg)
{
  while (1)
  {
    pthread_mutex_lock(&mutex_a); // Note the lock order is the same as thread 1
    pthread_mutex_lock(&mutex_b); // Note the lock order is the same as thread 1
